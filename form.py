...
class Form():
	...
	def create_text_input(self, label_:str, help_:str= None, space:bool=True, value_:str = "", key_= None):
		st.markdown(label_, help = help_)
		if key_ is None: 
			key_= label_
		value = st.text_input("", label_visibility = "collapsed", key = key_, value = value_)
		if space:
			st.text("")
		return value 
	
	def create_number_input(self, label_:str, min_=0, val_= 0,help_:str=None, space:bool=True, max_= None, step_=1):
		st.markdown(label_, help = help_)
		value = st.number_input("",min_value=min_, max_value= max_, step= step_, value=val_, label_visibility = "collapsed", key = label_)
		if space:
			st.text("")
		return value 
	...
	def create_page(self):
		...
		preprocessing_json = self.create_text_input("Enter the name of the json file.",help_=help_json)
		...
		lbl_maps_dir =("Give a path to a new MAPS folder where the results will be stored")
		...
		dropout = self.create_number_input("Chose the rate of dropout applied in dropout layers.", min_=0.0, max_=1.0, val_ =0.0, step_ = 0.1)
		...
		diag_input = self.create_text_input("If you need other diagnoses than AD or CN please enter them one by one here:")
		diag_list = ["AD", "CN"]
		if diag_input is not '' :
			add_diag = True
			diag_list.append(diag_input)
			while add_diag :
				add_diag = False
				diag_input2 = self.create_text_input("Enter here other diagnosis:", key_=diag_input+f"{len(diag_list)}")
				if diag_input2 is not '' :
					diag_list.append(diag_input2)
					add_diag = True

		diagnoses = st.multiselect("Enter the list of the files which will be used for training. Default will look for AD and CN TSV files.",diag_list,default = ("AD", "CN"))
		st.sidebar.write(f"**diagnoses**: {diagnoses}")
		self.write_dict(section_, "diagnoses", diagnoses)
